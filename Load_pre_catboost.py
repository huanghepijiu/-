...
# 引入数据
import Load_data_CatBoost_Data as data
...
def Train(data, modelcount, censhu, yanzhgdata, predata, laidex=label):
    ...
    #  开始训练数据
    model.fit(data[:, :6], data[:,6], cat_features=laidex, eval_set=(yanzhgdata[:,:6], yanzhgdata[:,6]))

    # 给出训练数据的评分
    train_mse = model.score(data[:, 0:6], data[:,6])

    # 计算预测数据的评分
    add_mse = model.score(yanzhgdata[:,0:6], yanzhgdata[:,6])

    print(train_mse, add_mse)
    return train_mse, add_mse, model.predict(predata[:,0:6])
...
def Pailie(realr, modelout, count=90):
    ...
    # 随机选取
    fu = np.random.choice(list(range(len(realr))), count, replace=True)
...
def duibi(exdict, you, kaudu=0.35):
    ...
    plt.savefig(r'C:\Users\黄河脾酒\Desktop\CatBoost_pm25.jpg')
    return '不同方法对比完毕'
...
def recspre(yzhenshide, yyucede):
    #  获得展示的数据
    yreal, ypre, cha = Pailie(yzhenshide['predict'][:,6], yyucede)
    ...
    plt.savefig(r'C:\Users\黄河脾酒\Desktop\CatBoost_duibi.jpg')
